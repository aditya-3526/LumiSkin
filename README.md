...
🔗 **Interactive Tableau Dashboard:** *Coming soon — currently being built using the Tableau export package generated by Component 7.*
...
*Customer segments sized by total CLV — Champions and Loyal Customers represent under 2% of the base but disproportionately concentrated value.*

![Segment overview showing customer segments sized by total CLV contribution](outputs/charts/segment_overview.png)

*Current uniform spend vs proposed segment-differentiated allocation — the reallocation away from Lost Customers drives the 131% ROI uplift.*

![Retention ROI comparison between current uniform and proposed segment-differentiated budget allocation](outputs/charts/retention_roi_comparison.png)

*Within-segment CLV variance shown on a log scale — even within a single segment, customers vary by an order of magnitude, justifying CLV-ranked targeting inside each bucket.*

![CLV distribution within segments on log scale showing within-segment variance](outputs/charts/clv_distribution.png)
...
1. **Segment size distribution reflects dataset structure, not target-state economics** — The Olist dataset covers a 2-year window (2016–2018) with limited customer maturity, producing a 97% single-purchase rate. As a result, the Champions segment is intentionally small (27 customers, 0.03%) and the Lost Customers segment is intentionally large (61.9%). These thresholds are appropriate for the dataset and analytically defensible, but in a production deployment with 5+ years of customer history, segment proportions would naturally rebalance toward a more typical D2C distribution (Champions ~5–10%, Lost ~25–35%). The methodology is portable; the absolute segment sizes are dataset-specific.
2. **97% single-purchase base** — CLV estimates for the majority of customers rely on imputed assumptions, not observed behavior
3. **BG/NBD non-convergence** — the probabilistic model could not fit this dataset; empirical lifespan estimates are conservative but less precise
4. **Intervention assumptions** — recovery rates and conversion rates are based on industry benchmarks, not A/B test results
5. **Cross-channel effects** — referral value, social influence, and brand advocacy are not captured in CLV
6. **Static snapshot** — analysis uses a fixed reference date; production deployment would require automated refresh
